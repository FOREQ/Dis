...
import pandas as pd
# Убедитесь, что импортируются все нужные метрики, включая chrF и новый MAKTS
from metrics import (
    calculate_bleu,
    #calculate_ter,
    calculate_meteor,
    calculate_beer,
    calculate_chrf, # Добавлен chrF
    calculate_makts # Убедитесь, что это НОВАЯ реализация MAKTS v2
)
# Убедитесь, что импортируются все утилиты
from utils import (
    validate_input,
    create_metrics_chart,
    create_results_df,
    get_metric_descriptions # Должен содержать описание нового MAKTS
)

# --- Конфигурация страницы ---
st.set_page_config(
    page_title="Оценка Качества Перевода", # Название на русском
    page_icon="⚖️", # Эмодзи для иконки
    layout="wide" # Широкий макет
)

# --- Заголовок и Описание ---
st.title("Инструмент Оценки Качества Перевода")
# Обновленное описание, включающее все метрики
st.markdown("""
...
**BLEU**, **chrF**, **TER**, **METEOR**, **BEER** и **MAKTS v2** (chrF с весами для корней слов).
...
# Определяем исходный язык для BLEU (хотя текущая реализация BLEU его не использует)
    ...
    with col1:
        ...
        if reference_file:
            try:
                ...
                # Показываем загруженный текст (для проверки)
    ...
    if not is_ref_valid or not is_cand_valid:
        ...
        with st.spinner("Вычисление метрик... Это может занять некоторое время, особенно для MAKTS."):
            try:
                ...
                # Можно добавить настройку root_weight в интерфейсе, если нужно
                ...
                # Убираем None, если какая-то метрика вернула None (маловероятно теперь)
                scores_filtered = {k: v for k, v in scores.items() if v is not None}

                 # Готовим данные для диаграммы (TER может быть инвертирован или отфильтрован в utils.py)
                ...
                with col_res2:
                    # Радарная диаграмма
                    if scores_for_chart:
                        # Функция create_metrics_chart может обрабатывать TER для диаграммы
                ...
                # Загружаем описания (включая новое для MAKTS)
                descriptions = get_metric_descriptions()

                # Отображаем описания для посчитанных метрик
                displayed_metrics = list(scores_filtered.keys())
                # Сортируем для консистентного порядка (опционально)
                # displayed_metrics.sort()

                for metric in displayed_metrics:
                     if metric in descriptions:
                         with st.expander(f"Подробнее о {metric}"):
                             # Используем markdown для форматирования описаний
                             st.markdown(descriptions[metric], unsafe_allow_html=True) # Разрешаем HTML, если нужно
                     else:
